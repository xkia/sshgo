...
| `host_manager.py` | `HostManager` class — public facade for config lifecycle, stable node ID assignment, CRUD, validation delegation, alias lookup, connection execution, and CLI preview launch args |
| `connection_errors.py` | Shared connection/config runtime exceptions |
| `endpoint.py` | Pure endpoint helpers — `DEFAULT_PORT`, host-address validation without embedded ports, and host/port formatting used by validation, planning, titles, and Recent |
...
| `audit_logger.py` | `AuditLogger` class — manages runtime data dir (`~/.sshgo/` or `$SSHGO_DATA_DIR`), writes history and optional full-audit JSONL with node identity/endpoint fields and retention limits (history: 5000, audit-full: 2000) |
...
5. **Recent Resolution**: TUI builds the Recent group by walking retained history newest-first and keeping the most recent distinct hosts (up to the display limit); start events and exec failures both count. It resolves current nodes by `node_id` first, then legacy name/endpoint fields, and only falls back to read-only history snapshots when the configured node no longer exists.

6. **Credential ownership**: `password` and `mfa_secret` are optional plain config values. sshgo does not encrypt them at rest; users own config/backup permissions or can rely on keys, SSH agent, and manual prompts. Active legacy encryption is rejected with migration guidance.
...
- **Audit logging**: JSONL files in `~/.sshgo/`. History is always written for SSH and SFTP starts, including `sftp_interactive_started` and exec failures; audit-full requires the `--audit-full` flag and adds extended launch context. New records include `node_id`, `port`, and `endpoint`. Because of the execve handoff, final duration, exit code, and commands typed inside `sftp>` are not available in current audit records.
