...
| R6 | HostManager and entry-point scope | Medium | reduced | `HostManager` still owns domain behavior, but command planning, config storage, config validation, and pure host-tree operations are isolated behind focused helpers with dedicated tests. CLI diagnostics/config helper code is now outside `sshgo.py`, and stale write detection prevents older HostManager instances from silently overwriting newer config saves. | Covered by [command-planning-extraction](specs/command-planning-extraction.md), [config-store-extraction](specs/config-store-extraction.md), [config-validation-extraction](specs/config-validation-extraction.md), [host-tree-extraction](specs/host-tree-extraction.md), [project-cohesion-refactor](specs/project-cohesion-refactor.md), and [final-risk-hardening](specs/final-risk-hardening.md). |
| R7 | Config validation depth | Medium | closed | Known top-level config fields and node candidates are validated through a focused validator module; unknown top-level config keys remain allowed for compatibility. | Covered by [config-and-audit-durability](specs/config-and-audit-durability.md), [common-workflow-polish](specs/common-workflow-polish.md), and [config-validation-extraction](specs/config-validation-extraction.md). |
| R8 | TUI lifecycle and rendering | Low | reduced | Shared TUI templates, in-form validation, cursor-aware field editing, editable-only parent selection, and pure text/form helper modules reduce daily friction; terminal-specific curses edge cases remain possible. | Covered by [tui-style-system](specs/tui-style-system.md), [tui-form-interaction-polish](specs/tui-form-interaction-polish.md), [tui-input-editing-polish](specs/tui-input-editing-polish.md), [project-cohesion-refactor](specs/project-cohesion-refactor.md), and [final-risk-hardening](specs/final-risk-hardening.md). |
...
| Config validation extraction | Parsed-config validation is testable outside HostManager while keeping compatibility imports and existing rules. | [config-validation-extraction](specs/config-validation-extraction.md) |
...
| Project cohesion refactor | CLI diagnostics/config helpers, pure TUI text/form helpers, and SFTP/relay transfer coverage are organized by responsibility without changing behavior. | [project-cohesion-refactor](specs/project-cohesion-refactor.md) |
