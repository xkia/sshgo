...
| R6 | HostManager scope | Medium | reduced | `HostManager` still owns domain behavior, but command planning, config storage, config validation, and pure host-tree operations are isolated behind focused helpers with dedicated tests. Stale write detection prevents older HostManager instances from silently overwriting newer config saves. | Covered by [command-planning-extraction](specs/command-planning-extraction.md), [config-store-extraction](specs/config-store-extraction.md), [config-validation-extraction](specs/config-validation-extraction.md), [host-tree-extraction](specs/host-tree-extraction.md), and [final-risk-hardening](specs/final-risk-hardening.md). |
...
| R8 | TUI lifecycle and rendering | Low | reduced | Shared TUI templates, in-form validation, cursor-aware field editing, and editable-only parent selection reduce daily friction; terminal-specific curses edge cases remain possible. | Covered by [tui-style-system](specs/tui-style-system.md), [tui-form-interaction-polish](specs/tui-form-interaction-polish.md), [tui-input-editing-polish](specs/tui-input-editing-polish.md), and [final-risk-hardening](specs/final-risk-hardening.md). |
